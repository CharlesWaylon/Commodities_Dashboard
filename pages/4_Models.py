...
def _render_model_health():
    ...
    elif _trend_df["computed_at"].nunique() < 2:
        _ph_trend.empty()
        _single_day = pd.to_datetime(_trend_df["computed_at"].iloc[0]).strftime("%b %d, %Y")
        _mean_ic_all = _trend_df["mean_ic"].mean()
        st.info(
            f"Only one day of IC data ({_single_day}) — mean IC across tiers: **{_mean_ic_all:+.4f}**. "
            "Re-run retraining on different days to build a trend.",
            icon="📈",
        )
    else:
        ...
        with st.expander("📈 IC-vs-threshold curves", expanded=False):
            ...
            for _, _tr_row in _latest_tune.iterrows():
                ...
                try:
                    ...
                    if not _grid:
                        continue
                    _t_vals  = sorted(float(k) for k in _grid.keys())
                    _ic_vals = [float(_grid[str(t)]) if str(t) in _grid else float(_grid.get(t, float("nan"))) for t in _t_vals]
                    ...
                    st.caption(f"⚠️ Could not render curve for {_fam}: {_curve_err}")
            if _curves_rendered == 0:
                st.info(
                    "No IC curves available yet — click **⚙️ Tune Thresholds** above "
